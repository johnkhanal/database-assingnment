...
BEGIN 
	...
	ELSE
		...
		DBMS_OUTPUT.PUT_LINE('THE TOTAL REVENUE GENERATED FOR THIS EVENT IS '||rec_cur_fest_detail.festival_name||' is Rs.'||vn_expense);
...
BEGIN 
	...
	IF cur_expense%FOUND THEN
		...
		DBMS_OUTPUT.PUT_LINE('AMOUNT SPENT ON STAFF SALARY FOR EVENT '||in_festival_staff_id||' Rs.'||vn_salary||'\- TOTAL RENT FOR THE LOCATION IS Rs.'||vn_rent||'\- TOTAL COST FOR THIS EVENT IS Rs.'||vn_expense||'\-');
	ELSE 
